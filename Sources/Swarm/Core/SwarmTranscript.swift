...
public struct SwarmTranscript: Codable, Sendable, Equatable {
    ...
    /// Validates that this transcript can be replayed with the expected schema version.
    ///
    /// Use this method to check transcript compatibility before attempting replay
    /// operations, such as restoring agent state from a checkpoint.
    ///
    /// - Parameter expected: The expected schema version. Defaults to the current version.
    /// - Throws: `SwarmTranscriptReplayCompatibilityError.incompatibleSchemaVersion`
    ///   if the transcript's schema version doesn't match the expected version.
    ///
    /// ## Example
    ///
    /// ```swift
    /// let transcript = SwarmTranscript(memoryMessages: messages)
    ///
    ...
    ///     // Proceed with replay
    /// } catch {
    ///     print("Cannot replay: incompatible schema version")
    /// }
    /// ```
    ...
    /// Returns a stable, deterministic binary representation of the transcript.
    ///
    /// This method produces consistent data output for the same transcript content,
    /// making it suitable for hashing and integrity verification. The encoding
    /// uses sorted keys and no escaping slashes to ensure stability.
    ///
    /// - Returns: JSON-encoded data representing the transcript.
    ...
    ///
    /// // Store or transmit the stable representation
    ...
    /// Computes a SHA-256 hash of the transcript for integrity verification.
    ///
    /// This hash can be used to:
    /// - Verify transcript integrity during storage or transmission
    /// - Detect tampering or corruption
    /// - Create unique identifiers for transcript states
    ///
    /// - Returns: A hexadecimal string representing the SHA-256 hash.
    /// - Throws: An error if stable data encoding fails.
    ///
    /// ## Example
    ///
    /// ```swift
    ...
    ///
    /// print("Transcript hash: \(hash)")
    /// // Use hash for integrity checks or storage indexing
    /// ```
    ...
    /// Finds the first difference between this transcript and another.
    ///
    /// Compares transcripts entry by entry to identify where they diverge.
    /// Useful for debugging, testing, and verifying transcript integrity
    /// after modifications.
    ///
    /// - Parameter other: The transcript to compare against.
    /// - Returns: A `SwarmTranscriptDiff` describing the first difference found,
    ///   or `nil` if the transcripts are identical.
    ///
    /// ## Example
    ///
    /// ```swift
    /// let original = SwarmTranscript(memoryMessages: messages1)
    /// let modified = SwarmTranscript(memoryMessages: messages2)
    ///
    /// if let diff = original.firstDiff(comparedTo: modified) {
    ///     print("Difference at entry \(diff.entryIndex)")
    ///     print("Key path: \(diff.keyPath)")
    ///     print("Original: \(diff.lhs)")
    ///     print("Modified: \(diff.rhs)")
    /// } else {

...
| [Foundation Models modes](/guide/foundation-models) | Capture vs provider-owned tool loop; OS 27 PCC via `.foundationModels(model:)`; owned-loop `reasoningLevel` and Swarm profile / history → `Transcript`; `.specific` tool-choice stays prompt-only |

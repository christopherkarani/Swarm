...
struct InferenceMessageAttachmentTests {
    ...
    func codecOmitsAudioWithoutCapability() throws {
        let message = InferenceMessage.user("hi", attachments: [audioAttachment])
        ...
        #expect(omitted.content == .text("hi"))
        let data = try OpenAICompatibleWire.encode(included)
        let decoded = try JSONSerialization.jsonObject(with: data) as? [String: Any]
        let parts = decoded?["content"] as? [[String: Any]]
